...
public class ProductbatchCompDAO implements IProductbatchCompDAO {
	...
	public ProductbatchCompDTO getProductbatchComp(int pbID, int mbID) throws DALException {
		try {
			PreparedStatement ps = Connector.prepare("SELECT user_id, tare, netto FROM productbatch_component WHERE pb_id = ? && mb_id = ?");
			ps.setInt(1, pbID);
			ps.setInt(2, mbID);
			...
			if (!rs.first()) {
				...
				return new ProductbatchCompDTO(pbID, mbID, rs.getInt("user_id"), rs.getDouble("tare"), rs.getDouble("netto"));
			}
	...
	public List<ProductbatchCompDTO> getProductbatchCompList() throws DALException {
		...
		try {
			ResultSet rs = Connector.doQuery("SELECT pb_id, mb_id, user_id, tare, netto FROM productbatch_component");
			while (rs.next()) {
				list.add(new ProductbatchCompDTO(rs.getInt("pb_id"), rs.getInt("mb_id"), rs.getInt("user_id"), rs.getDouble("tare"), rs.getDouble("netto")));
			}
		} catch (SQLException e) {
			throw new DALException(e);
		}
	...
	public List<ProductbatchCompDTO> getProductbatchCompList(int pbID) throws DALException {
		List<ProductbatchCompDTO> list = new ArrayList<ProductbatchCompDTO>();
		try {
			PreparedStatement ps = Connector.prepare("SELECT mb_id, user_id, tare, netto FROM productbatch_component WHERE pb_id = ?");
			ps.setInt(1, pbID);
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				list.add(new ProductbatchCompDTO(pbID, rs.getInt("mb_id"), rs.getInt("user_id"), rs.getDouble("tare"), rs.getDouble("netto")));
			}
		} catch (SQLException e) {
			throw new DALException(e);
		}
	...
	public void createProductbatchComp(ProductbatchCompDTO productbatchComponent) throws DALException {
		try {
			PreparedStatement ps = Connector.prepare("INSERT INTO productbatch_component(pb_id, mb_id, user_id, tare, netto) VALUES (?,?,?,?,?)");
			ps.setInt(1, productbatchComponent.getPbID());
			ps.setInt(2, productbatchComponent.getMbID());
			ps.setInt(3, productbatchComponent.getUserID());
	...
	public void updateProductbatchComp(ProductbatchCompDTO productbatchComponent) throws DALException {
		try {
			PreparedStatement ps = Connector.prepare("UPDATE productbatch_component SET tare = ?, netto = ?, user_id = ? WHERE pb_id = ? AND mb_id = ?");
